...
ifdef OS
    # Windows
    RM = del /Q
    NormalizePath = $(subst /,\,$1)
    BASH = C:/Windows/System32/bash.exe -c
else
    # Unix
...
mv3-firefox: tools/make-mv3.sh $(sources) $(platform)
	$(BASH) tools/make-mv3.sh firefox

mv3-quick: tools/make-mv3.sh $(sources) $(platform)
	$(BASH) tools/make-mv3.sh quick

mv3-full: tools/make-mv3.sh $(sources) $(platform)
	$(BASH) tools/make-mv3.sh full

dist/build/uAssets:
	$(BASH) tools/pull-assets.sh

clean:
	$(RM) dist/build tmp/node_modules
